...
(function (global) {
  ...
  /* Composite RANK score: survival(0.45) + lab-trust(0.35) + value(0.20). Value is
     cost-per-expected-live-plant, clamped [0,100] so a cheap-but-doomed cup can't win. */
  function score(o, skill) {
    var s = survival(o, skill);
    var survScore = s.pAtLeastOne * 100;
    var priceScore = Math.max(0, Math.min(100, 100 - (s.costPerLive - 15) * 1.5));
    return 0.45 * survScore + 0.35 * labTrust(o) + 0.20 * priceScore;
  }
  ...
  /* Recompute survival/trust/score for `skill`, sort by score desc, apply the
     #1-slot rule. Sets o._s / o._lt / o._score in place (the UI reads o._s). */
  function recompute(arr, skill) {
    arr.forEach(function (o) { o._s = survival(o, skill); o._lt = labTrust(o); });
    arr.forEach(function (o) { o._score = score(o, skill); });
