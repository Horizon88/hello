...
def check_sellers(sellers):
    for s in sellers or []:
        ...
        if not isinstance(lg, dict):
            ...
            continue
        for f in ("shipments", "honored_claims", "checkins_responded",
        ...
        # Guard only if the four fields are individually valid ints.
        if all(is_int(lg.get(f)) for f in
               ("shipments", "honored_claims", "checkins_responded",
                "establishment_confirmed")):
...
def enrich(offer, seller):
    ...
    if lg and lg.get("shipments", 0) > 0 and lg.get("checkins_responded", 0) > 0:
        o["contam"] = beta_mean(lg["honored_claims"], lg["shipments"],
        ...
        o["contam"] = seller.get("contamination_rate_claimed", 0.15)
        o["contam_deflask"] = seller.get("deflask_success_rate_claimed", 0.6)
...
def lab_trust(o):
    ...
    raw = (0.32 * contam_score + 0.20 * deflask_score + 0.14 * rating_score
           + 0.10 * vol_score + 0.12 * cert_score + 0.12 * (1 if o["replace"] else 0))
...
def score(o, skill):
    ...
    price_score = max(0, 100 - (s["costPerLive"] - 15) * 1.5)
    lt = lab_trust(o)
...
def check_golden_ranking(offers, sellers):
    ...
    for o in pool:
        ...
        eo = enrich(o, s)
        ranked.append((score(eo, GOLDEN_SKILL), eo))
    ranked.sort(key=lambda x: x[0], reverse=True)
    order = [eo["handle"] for _, eo in ranked]
    ...
    if gamer:
        ...
        if lg.get("shipments"):
            witnessed = lg["honored_claims"] / lg["shipments"]
...
def main():
    ...
    print("  sellers  checked: %d (ledger sanity ok)" % (len(sellers) if sellers else 0))
